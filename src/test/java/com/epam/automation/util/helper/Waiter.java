...
public class Waiter extends Helper {
    ...
    public static void waitUntilAttributeAriaExpandedToBe(WebElement mdSelect, Boolean attributeValue) {
        driverWait
                .until(ExpectedConditions.attributeToBe(mdSelect, ARIA_EXPANDED_ATTRIBUTE, attributeValue.toString()));
    }

    public static void waitForFrame(String frameName) {
        new WebDriverWait(driver, Duration.ofSeconds(WAITING_PERIOD))
                .until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(frameName));
    }

    public static void waitForFrame(int frameNumber) {
        new WebDriverWait(driver, Duration.ofSeconds(WAITING_PERIOD))
                .until(ExpectedConditions.frameToBeAvailableAndSwitchToIt(frameNumber));
    }

    public static void waitUntilNewWindowHandleAppear() {
        driverWait.until(driver -> driver.getWindowHandles()
                .size() == EXPECTED_WINDOW_HANDLES_COUNT);
    }
